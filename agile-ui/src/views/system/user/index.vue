              ...
              <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['system:user:resetPwd', 'system:user:edit']">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item command="handleAuthRole" icon="el-icon-circle-check"
                    v-hasPermi="['system:user:edit']">Assign Permissions</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="Department" prop="deptId">
              <treeselect v-model="form.deptId" :options="deptOptions" :show-count="true" placeholder="Department" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="Phone" prop="phonenumber">
              <el-input v-model="form.phonenumber" placeholder="Phone" maxlength="11" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="Gender">
              <el-select v-model="form.sex" placeholder="Gender">
                <el-option
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      ...
      >
        ...
        <div class="el-upload__tip text-center" slot="tip">
          ...
          <span>Only xls and xlsx files are supported.</span>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 6, label: `CreationTime`, visible: true }
      ],
      // 表单校验
      rules: {
        userName: [
        ...
        password: [
          ...
          { min: 5, max: 20, message: 'Password length must be 5-20 characters', trigger: 'blur' },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('Confirm to "' + text + '" the user "' + row.userName + '"?').then(function() {
        return changeUserStatus(row.userId, row.status);
      }).then(() => {
        this.$modal.msgSuccess(text + "Done!");
      }).catch(function() {
    ...
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
      this.$prompt('Please enter a new password for"' + row.userName, "Notice", {
        confirmButtonText: "Confirm",
        cancelButtonText: "Cancel",
        closeOnClickModal: false,
        inputPattern: /^.{5,20}$/,
        inputErrorMessage: "Password length must be 5-20 characters",
        inputValidator: (value) => {
          ...
          resetUserPwd(row.userId, value).then(response => {
            this.$modal.msgSuccess("Password updated. New password:" + value);
          });
        }).catch(() => {});
    },
    /** 分配角色操作 */
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('Confirm deletion of the data item with user ID "' + userIds + '"?').then(function() {
